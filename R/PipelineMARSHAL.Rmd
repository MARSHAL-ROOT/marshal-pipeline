...
To illustate the potential of MARSHAL, we will create 
...
Thanks to the details information, with can mesures some root phenes along the z axis. The maximal rooting depth is a interesting phenotypic values. This can even be use in other models such as Feddes et al. [-@feddes1978] to estimate root water uptake.
...
Root length density is a major phenotypic trait mesured in field when below ground information are requested. Here we computed the total root length every ten days. So, we can observe that after 60 days, the total root length is more less the same for the three RSA example.
...
```{r krs, echo = F, warning = F, fig.cap="Evolution of the whole root system conductance. Effect comparison between root system architecture and root conductivity modification.", fig.pos = "!h", fig.height=4, fig.width= 8}
results%>%
...
```{r transpi with water, echo = F, warning= F, fig.pos = "!h", fig.height=4}
...
```{r transpi with dry condition, echo = F, warning = F, fig.pos = "!h", fig.height=4}
