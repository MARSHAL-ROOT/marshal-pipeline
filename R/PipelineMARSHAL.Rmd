...
```{r , echo = F, fig.cap="Convex hull of the root system examples x and y axes together" }
pol_ch%>%
  ggplot()+
  theme_classic()+
  coord_fixed() +
...
```{r suf on roots, echo=FALSE, fig.cap="Standard uptake fraction of the different root systsem architecture"}
...
new_all_roots%>%
  ...
  theme_classic()+
  coord_fixed() + 
  geom_segment(data = , aes(x = x1, y = z1, xend = x2, yend = z2, colour = log10(suf1)), alpha = 0.8)+
...
#### Where are the segment with the highest contribution over the SUF.

One of the major advantage of the SUF is to estimate where should be the segment that contribute to the water uptake of the plant.The smallest region on which more or less 50% of the water uptake occurs in a soil where the water potential is homogenous along the profile can be calculated.
...
```{r suf1 on Postma 2011 rsa, echo=FALSE, fig.cap="The smallest region on one Postma 2011 RSA example which contains 50 % of the SUF"}
...
```{r suf1 on Leitner 2010 rsa, echo=FALSE, fig.cap="The smallest region on one Leitner 2010 RSA example which contains 50 % of the SUF"}
...
```{r suf1 on Leitner 2014 rsa, echo=FALSE, fig.cap="The smallest region on one Leitner 2014 RSA example which contains 50 % of the SUF"}
...
```{r jr on roots, echo=FALSE, error= F, warning=F, fig.cap="Absorption speed, comparison between the RSA and the soil scenario"}
new_all_roots%>%
  filter(age == "60",
         rep == nrep,
         scenario == 1) %>%
  ...
  theme_classic()+
  coord_fixed() + 
  geom_segment(data = , aes(x = x1, y = z1, xend = x2, yend = z2, colour = log10(jr)), alpha = 0.8)+
...
```{r jxl on roots, echo=FALSE, warning=F, error= F, fig.cap="Absorption speed, comparison between the RSA and the soil scenario"}
...
new_all_roots%>%
  ...
  theme_classic()+
  coord_fixed() + 
  geom_segment(data = , aes(x = x1, y = z1, xend = x2, yend = z2, colour = log10(jxl)), alpha = 0.8)+
  facet_grid(ex ~ humid, labeller = as_labeller(babbel)) + 
  scale_color_viridis_c()+
...
new_all_roots%>%
  ...
  theme_classic()+
  coord_fixed() + 
  geom_segment(data = , aes(x = x1, y = z1, xend = x2, yend = z2, colour = kx), alpha = 0.8)+
  facet_grid(ex~scenario, labeller = as_labeller(babbel)) + 
  scale_color_viridis_c()
```
...
```{r graph dens root, echo = F, warning=F, fig.cap="Root length density"}
dens%>%
  filter(age == 60,
         scenario == min(scenario),
         humid == min(humid))%>%
...
```{r dens suf 1, echo = F, warning=F, fig.cap="Evolution of the SUF with Leitner 2010 RSA parameter"}
dens%>%
...
```{r dens suf 2, echo = F, warning=F, fig.cap="Evolution of the SUF with Postma 2011 RSA parameter"}
dens%>%
...
```{r dens suf 3, echo = F, warning=F, fig.cap="Evolution of the SUF with Leitner 2014 RSA parameter"}
dens%>%
...
```{r dens radial 2, echo = F, fig.cap="Comparison of the radial fluxes between the different soil scenarios and the root system architectures"}
dens%>%
  filter(age == 60,
         scenario == 1
         #,ex == unique(dens$ex)[1]
...
```{r dens axial 2, fig.cap="Comparison of the axial fluxes between the different soil scenarios and the root system architectures"}
dens%>%
  filter(age == 40,
         scenario == 1)%>%
...
```{r psi, echo=F, warning= F, fig.cap="Water pressure head at the soil-root interface"}
dens%>%
  filter(age == 50,
         scenario == 2)%>%
...
```{r krs, echo = F, warning = F, fig.cap="Evolution of the whole root system conductance. Effect comparison between root system architecture and root conductivity modification."}
results%>%
...
```{r transpi with water, echo = F, warning= F}
...
```{r transpi with dry condition, echo = F, warning = F}
