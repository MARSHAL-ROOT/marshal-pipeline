...
The coupling of models with one another can be use to find releavant sequence of information. This can create a network of data that can be use to answer complex questions which interconnect multiscale or disciplinary frame [@Passot2018-ox]. Depending of the models, the complexity of the system is significantly increase. Here, the system on which this pipeline works is a below ground system. It focuses on solving the hydraulic architecture of a root system. To illustrate the necessity of merging models together, we start by the fact that **MARSHAL** need an explicit root system architecture to add on the hydraulic part. **CRootBox** is precisely a model that compute the explicit root system architecture [@CrootBox] and MARSHAL was designed to accept easily the CRootBox output. Dealing with models as an input for another model will also increase the dynamic approach of the overall [@Passot2018-ox]. The 
...
or by simply excecute the following line in the R environment:
...
MARSHAL is built upon a handfull of other packages that need to be loaded. One groupe of [function](https://github.com/MARSHAL-ROOT/marshal-pipeline/R/) has to be loaded manually.
...
babbel <- c("all_rain" = "rain all time",
           ...
           "50" = "50", "60"= "60",
           "Doussan et al. \n" = "Doussan et al.", "Early Doussan et al. \n" = "Early",
           "Late Doussan et al. \n" = "Late",
...
The bondary condition of MARSHAL are :
...
The hydraulic properties of the root system can be found in the litterature, by measuring it on root samples or with the helps of modelling tools as well. Here, the conductivities of the root come from the Doussan et al. paper [-@Doussan]. To show some possibilities, three scenarios in total were built from this Doussan et al. table of conductivities (figure \ref{conduct}). The first, which is the original values placed along the root axes from the apex. The second, is based on the first but the maturity of the root anatomy is advanced of 5 cm towards the apex. And the third, is the opposite of the second, or in other words, the maturity of the root anatomy, insted of being advance towards the apex, is moved of 5 cm backwards.
...
Modelling root system architecture can be achieve with CRootBox for instance [@CrootBox]. Some examples of the parametre files for a couples of specieses are details in the *modelparameter* folder, which is linked to the [CRootBox repository](https://github.com/Plant-Root-Soil-Interactions-Modelling/CRootBox).
...
The following lines of code allows to create five unique root system architectures for each parameter files. It computes the root growth during 60 days and load every ten days the architecture information in a data table. Inside the CRootBox implementation for this example, the root growth has been restricted to the below ground level for a more realistic point of view. Furthermore, some segments were not build up to the whole root system because they were shorter than the spatial resolution of the root system.
...
The figure \ref{roots} shows the root system architectures that come out CRootBox at the half of the simulation time (30 days).
...
Once the complete root system architecture is computed there are a number of architetural phenes that we can access.
...
here we summarize briefly with a couple of figure, the different architectural phenes which can be mesured.

#### The convex hull

The convex hull is one of the most commun known traits to works with. It gives a qualitative information about root exploration. To find the convex hull with an explicit root system in 3D is straight forwards. Thanks to the chull function, we are able to identify all the coordinates that make the convex hull and the Polygon function in the sp package allows us to determine the area of the convex hull.
...
Thanks to the details information, with can mesures some root phenes along the z axis. The maximal rooting depth is a interesting phenotypic values. This can even be use in other model such as hydrus1D to estimate root water uptake.
...
Root length density is a major phenotypic trait mesured in field when below ground information are requested. Here we computed the total root length every ten days. So, we can observe that after 60 days, the total root length is more less the same for the three RSA example. which can be approximate to a similar carbon investment in the root system at that time. King et al. [-@King2003] showed that a more homogenius vertical distribution of the roots should pay back in terms of carbon investments for water uptake. In the **result** section this assumption is developped. 

```{r root length, echo= F, fig.cap="The root lenght", fig.subcap=c("The total root length", "The root length density after 60 days."), fig.pos = "!h", out.width='.49\\linewidth', fig.width=3, fig.height=3,fig.show='hold',fig.align='center'}
RLD %>%
...
Although the total root length reach a close value of ten meter for all the different rootsystems after 60 days, there is a huge gap in term of number of roots between each of the root system architecture (figure \ref{numb roots}).

```{r fig.cap="Number of different roots for each of the root system examples. \\label{numb roots}", echo= F, fig.pos = "!h", fig.height = 2.5}
all_roots%>%
  group_by(age, ex, rep)%>%
...
Here, there are three soil scenarios with three different coeficients for the drying function.
...
Expressed in hecto-Pascal, the pressure head define the transpiration strenght that drags the water out of the root system. here we take the default value of $-15.000 \ hPa$
...
MARSHAL provide an estimation of the conductance of the whole root system, the actual and the potential transpiration.
...
The Standard uptake fraction is the relative distribution of root water uptake between root segments when water is equally available in space (units: %) [@Couvreur2012].

```{r suf on roots, echo=FALSE, fig.cap="Standard uptake fraction of the different root systsem architecture", fig.pos = "!h"}
...
#### Where are the segment with the highest contribution for the SUF.
...
# Distribution of the suf value
# fi%>%
#   ggplot(aes(x=1:nrow(fi), y = or_suf))+
...
```{r suf1 on Postma 2011 rsa, echo=FALSE, fig.cap="The smallest region on one Postma 2011 RSA example which contains 50 % of the SUF", fig.pos = "!h", fig.height= 5}
...
```{r suf1 on Leitner 2010 rsa, echo=FALSE, fig.cap="The smallest region on one Leitner 2010 RSA example which contains 50 % of the SUF", fig.pos = "!h", fig.height= 5}
...
```{r suf1 on Leitner 2014 rsa, echo=FALSE, fig.cap="The smallest region on one Leitner 2014 RSA example which contains 50 % of the SUF", fig.pos = "!h", fig.height = 5}
...
```{r jr on roots, echo=FALSE, error= F, warning=F, fig.cap="Absorption speed, comparison between the RSA and the soil scenario", fig.pos = "!h"}
new_all_roots%>%
  filter(age == 60,
         rep == nrep,
         scenario == "Doussan et al.") %>%
...
```{r jxl on roots, echo=FALSE, warning=F, error= F, fig.cap="Absorption speed, comparison between the RSA and the soil scenario", fig.pos = "!h"}
...
```{r kr on roots, echo=FALSE, warning = F, fig.cap="The radial conductivity (Kr) on root segment, comparison between the different conductivity scenario and main root system architecure examples", fig.pos = "!h"}
...
```{r kx on roots, echo=FALSE, warning = F, fig.pos = "!h", fig.cap="The axial conductance (Kx) on root segment, comparison between the different conductivity scenario and main root system architecure examples"}
...
### The evolution of the radial fluxes
...
```{r evolution jr on roots, echo=FALSE, warning=F, fig.pos = "!h", fig.cap = "Absorption dynamics due to different soil scenarios on the Postma root system architecture example"}
...
```{r evolution of jxl on roots, echo=FALSE, warning=F, fig.pos = "!h"}
...
```{r dens radial 2, echo = F, fig.cap="Comparison of the radial fluxes between the different soil scenarios and the root system architectures", fig.pos = "!h"}
dens%>%
  filter(age == 60,
         scenario == "Doussan et al."
         #,ex == unique(dens$ex)[1]
...
```{r dens axial 2, fig.cap="Comparison of the axial fluxes between the different soil scenarios and the root system architectures", fig.pos = "!h"}
dens%>%
  filter(age == 40,
         scenario == "Doussan et al.")%>%
...
```{r psi, echo=F, warning= F, fig.cap="Water pressure head at the soil-root interface", fig.pos = "!h"}
dens%>%
  filter(age == 50,
         scenario == "Doussan et al.")%>%
...
```{r krs, echo = F, warning = F, fig.cap="Evolution of the whole root system conductance. Effect comparison between root system architecture and root conductivity modification.", fig.pos = "!h", fig.height=5, fig.width= 8}
results%>%
...
```{r transpi with water, echo = F, warning= F, fig.pos = "!h", fig.height=6, fig.width=6}
...
```{r transpi with dry condition, echo = F, warning = F, fig.pos = "!h", fig.height=5}
