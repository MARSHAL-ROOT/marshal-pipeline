...
The hydraulic properties of the root system can be found in the litterature, by measuring it on root samples or with the helps of modelling tools as well. Here, the conductivities of the root come from the Doussan et al. paper [-@Doussan]. To show some possibilities, three scenarios in total were built from this Doussan et al. table of conductivities. The first, which is the original values placed along the root axes from the apex. The second, is based on the first but the maturity of the root anatomy is advanced of 5 cm towards the apex. And the third, is the opposite of the second, or in other words, the maturity of the root anatomy, insted of being advance towards the apex, is moved backwards.
...
```{r, results = 'asis', warning= F, echo=F, fig.pos="!h", fig.cap= "Conductivities properties on axes and lateral root type.", fig.subcap= c("Axial conductance.", "Radial conductivity."), fig.scap=NA, out.width='.40\\linewidth', fig.width=4, fig.height=4,fig.show='hold',fig.align='center'}
...
Inside the CRootBox implementation for this example, the root growth has been restricted to the below ground level for a more realistic point of view. Furthermore, some segments were not build up to the whole root system because they were shorter than the spatial resolution of the root system.
...
```{r , echo = F, fig.cap="Area of the convex hull for the different root system architecture examples. $\\bigtriangleup$ = Postma 2011, $\\square$ = Leitner 2014, and $\\bigcirc$ = Leitner 2010 RSA example", fig.subcap=c("Superposition of all the convex hull repetitions.", "The area of each convex hull."), fig.pos = "!h", out.width='.49\\linewidth', fig.width=3, fig.height=3,fig.show='hold',fig.align='center'}
pol_ch%>%
  ggplot()+
  theme_classic()+
  coord_fixed() +
...
```{r root length, echo= F, fig.cap="Total root length for each of the root system examples", fig.pos = "!h", fig.height= 2.5}
RLD %>%
...
```{r root tip number, echo= F, fig.cap="Number of different roots for each of the root system examples", fig.pos = "!h", fig.height = 2.5}
all_roots%>%
  group_by(age, ex, rep)%>%
...
Although the total root length reach a close value of ten meter for all the different rootsystems, there is a huge gap in term of number of roots between each of the root system architecture.
...
Here, there are three soil scenarios with three different coeficients in the drying function.
...
```{r , echo=F, fig.cap="Water potential evolution during the simultaion time along the soil profile", fig.pos = "!h", fig.height=2}
soil %>%
...
Expressed in hecto-Pascal, the pressure head define the transpiration strenght that drags the water out of the root system. $-15.000 \ hPa$
