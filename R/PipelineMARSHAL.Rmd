...
```{r , warning= F, echo=F, fig.pos="!h", fig.cap= "Axial conductance on axes and lateral root type", fig.height= 4}
...
```{r BC Kr, warning= F, echo= F, fig.cap= "Radial conductivity on axes and lateral root type", fig.height= 4}
...
```{r roots, echo=FALSE, fig.cap="Projection in two dimentions of the different root systems computed with CRootBox at the age of 30 days ", fig.pos = "!h", fig.height=9}
all_roots%>%
  filter(age == "30",
         rep < 6) %>%
  ggplot()+
...
The area shows a estimation of root exploration surface
...
```{r rooting depth, echo=F, fig.cap="Rooting depth of the different root system examples", fig.pos = "!h", fig.height=4}
...
```{r root tip number, echo= F, fig.cap="Number of different roots for each of the root system examples", fig.pos = "!h", fig.height = 4}
all_roots%>%
  group_by(age, ex, rep)%>%
...
```{r , echo=F, fig.cap="Water potential evolution during the simultaion time along the soil profile", fig.pos = "!h", fig.height=3}
soil %>%
